...
pipeline, extending \citet{wheeler2004}. We verify metadata extraction,
citation remapping, equation wrapping, figure and table formatting, list
flattening, and supplemental information handling.
\end{frame}
...
% Caption above the tabular (journal convention); converter wraps both in a
% table float with the caption kept above.
\captionof{table}{Comparison of forecast and observed amplitude statistics.}
\label{tab:summary}
\begin{tabular}{lcc}
\hline
...
% -----------------------------------------------------------------------
% Appendix  (AMS: after acknowledgments/data statement, before
% bibliography; AGU: before Open Research/COI/Acknowledgments -- opposite
% ends of the endmatter, each converter's own template order.)
% Two appendices -> auto-lettered A, B for both formats.
% -----------------------------------------------------------------------
\section{Appendix A: Extended Derivation}

\begin{frame}{Extended Derivation}
The full derivation proceeds by expanding the streamfunction in spherical
harmonics and retaining only the leading zonal wavenumbers.
\end{frame}
...
\begin{itemize}
  \item Zero flux at the domain edges
  \item Periodic in the zonal direction
\end{itemize}
\end{frame}
...
\fig{test_appendix_figure.pdf}
\captionof{figure}{Field significance test results for the appendix analysis.}
\end{frame}
...
\begin{frame}{SI: Two-panel composite}
  \includegraphics[width=0.49\linewidth]{test_si_panel_a.pdf}
  \includegraphics[width=0.49\linewidth]{test_si_panel_b.pdf}
\end{frame}
